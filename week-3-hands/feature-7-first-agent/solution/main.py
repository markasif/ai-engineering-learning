"""
Feature 7: First Agent — solution

Week 3 begins. Your assistant now has HANDS — it can take actions, not just
answer questions. This feature introduces tool calling and the agent loop.

New endpoints vs Feature 6:
  POST /api/agent/run   — run the agent loop for one message

The agent loop (in shared/agent.py):
  1. Send the user message + available tool schemas to the LLM.
  2. If the LLM calls tools: execute each via TOOLS_REGISTRY, collect results.
  3. Call the LLM AGAIN with the tool results to synthesize a final answer.
  4. Return {result, steps, tools_used} — the "show your work" response.

If the LLM decides no tools are needed, it answers directly without a second call.

All Feature 1-6 endpoints remain unchanged.

Run with:
    uvicorn main:app --reload --port 8000

YOUR DOMAIN CUSTOMIZATION:
  Replace the three example tools in shared/tools.py with tools relevant to
  your domain — keep the same function + TOOL_SCHEMA pattern.
"""
import json
import sys
import uuid
...
# ---------------------------------------------------------------------------
# Feature 7: Agent — the new endpoint
# ---------------------------------------------------------------------------
...
async def agent_run(
    ...
    The agent:
      1. Classifies whether a tool is needed.
      2. If yes: calls the LLM with tool schemas, executes the chosen tool(s),
         then calls the LLM again to synthesize a natural-language answer.
      3. If no: answers directly from the model's knowledge.
